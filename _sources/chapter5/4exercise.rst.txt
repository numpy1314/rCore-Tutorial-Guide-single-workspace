...
- 通过所有测例。
  ...
  在 os 目录下 ``make run BASE=2`` 加载所有测例，然后在终端中输入 ``ch5_usertest`` 运行，这个测例打包了所有你需要通过的测例。
  ...
  从本章开始，你的内核必须前向兼容。上文提到的无需再维护的 ``sys_trace`` 涉及的 ``ch3_trace`` 和 ``ch4_trace1`` 两个测例会被删除，除此之外，你的内核需要能通过前一章的所有测例。

.. caution:: 注意，由于本章内核默认将  ``ch5b_initproc`` 作为初始测例运行，所以必须用 ``BASE=2`` 而非之前章节的 ``BASE=0`` 加载。
